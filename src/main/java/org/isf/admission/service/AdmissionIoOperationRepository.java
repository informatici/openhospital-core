...
public interface AdmissionIoOperationRepository extends JpaRepository<Admission, Integer>, AdmissionIoOperationRepositoryCustom {
	...
	@Query(value = "select a FROM OH_Admission a WHERE a.admitted = 1 AND a.ward.code = :ward")
	List<Admission> findAllWhereWard(@Param("ward") String ward);

	@Query(value = "select a FROM OH_Admission a WHERE a.patient.code = :patient and a.deleted='N' and a.admitted = 1")
	Admission findOneWherePatientIn(@Param("patient") Integer patient);

	@Query(value = "select a FROM OH_Admission a WHERE a.patient.code = :patient and a.deleted='N' order by a.admDate asc")
	List<Admission> findAllWherePatientByOrderByDate(@Param("patient") Integer patient);

	@Query(value = "select a FROM OH_Admission a " +
	...
	@Query(value = "select a FROM OH_Admission a WHERE a.admitted =1 and a.ward.code = :ward and a.deleted = 'N'")
