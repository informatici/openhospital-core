...
public interface VisitsIoOperationRepository extends JpaRepository<Visit, Integer> {
	...
	@Query(value = "SELECT * FROM VISITS WHERE VST_WRD_ID_A = :ward AND VST_ACTIVE=1 ORDER BY VST_DATE", nativeQuery = true)
	List<Visit> findAllWhereWardByOrderDateAsc(@Param("ward") String ward);

	@Query(value = "SELECT * FROM VISITS WHERE VST_PAT_ID = :patient AND VST_ACTIVE=1 ORDER BY VST_PAT_ID, VST_DATE", nativeQuery = true)
