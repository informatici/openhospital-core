...
public class MedicalStockIoOperations {
			...
			String lotCode) throws OHServiceException
	{
		...
		Lot lot = (Lot)lotRepository.findById(lotCode).orElse(null);
		movement.setLot(lot);
	...
	protected String generateLotCode() throws OHServiceException
	{
		...
		do 
		{
			...
			lot = (Lot)lotRepository.findById(String.valueOf(candidateCode)).orElse(null);
		} while (lot !=null); 
			...
			double incrementQuantity) throws OHServiceException
	{
		...
		Medical medical = (Medical)medicalRepository.findById(medicalCode).orElse(null);
