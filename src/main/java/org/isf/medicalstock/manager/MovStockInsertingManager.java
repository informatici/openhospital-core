...
public class MovStockInsertingManager {
	...
	public MovStockInsertingManager(MedicalStockIoOperations medicalStockIoOperations,
	 ...
	 * @param checkReference - if {@code true} it will use
	 *                       {@link #checkReferenceNumber(String)
	...
	protected void validateMovement(Movement movement, boolean checkReference) throws OHServiceException {
		...
		if (lastDate != null && movDate.isBefore(lastDate)) {
			errors.add(new OHExceptionMessage(MessageBundle
					.getMessage("angal.medicalstock.multiplecharging.datecannotbebeforelastmovementdate.msg")));
		}
		 ...
		 * 
		 * Depending on the operation type and the modality
		 * AUTOMATICLOT_IN/AUTOMATICLOT_OUT, some properties will be checked.
		 * 
		 * - charge:
		 * 
		 * AUTOMATICLOT_IN=no, Lot(String, GregorianCalendar, GregorianCalendar) check
		 * everything
		 * 
		 * AUTOMATICLOT_IN=yes, Lot("", GregorianCalendar, GregorianCalendar) check
		 * everything but the code, it will be generated
		 * 
		 * - discharge:
		 * 
		 * AUTOMATICLOT_OUT=no, Lot(String, GregorianCalendar, GregorianCalendar) check
		 * everything
		 * 
		 * AUTOMATICLOT_OUT=yes, Lot("", null, null) no checks, the lot will be
		...
		if (lot != null) {
			...
			if (movement.getMedical() != null
					&& !(medicalIds.isEmpty() || medicalIds.size() == 1 && medicalIds.get(0).intValue() == movement
			...
			if (isCharge && GeneralData.LOTWITHCOST) {
				...
				if (cost == null || cost.doubleValue() <= 0.0) {
					errors.add(new OHExceptionMessage(MessageBundle
							.getMessage("angal.medicalstock.multiplecharging.zerocostsarenotallowed.msg")));
				}
			}
			 ...
			 * Check quantity.
			 * 
			 * AUTOMATICLOT_OUT=no, specified quantity must be equal or lower than the lot
			 * quantity
			 * 
			 * AUTOMATICLOT_OUT=yes, no check: the quantity will be split automatically
			 * between available lots
			 */
			if (!isAutomaticLotOut()) {

				if (movement.getType() != null && !isCharge && movement.getQuantity() > lot.getMainStoreQuantity()) {
					errors.add(new OHExceptionMessage(MessageBundle
							.getMessage("angal.medicalstock.movementquantityisgreaterthanthequantityof.msg")));
				}
			}
	...
	private void validateLot(List<OHExceptionMessage> errors, Lot lot, boolean checkCode) {
		...
		if (lot.getPreparationDate() != null && lot.getDueDate() != null
				&& lot.getPreparationDate().compareTo(lot.getDueDate()) > 0) {
			errors.add(new OHExceptionMessage(
	...
	/**
	 * Verify if the referenceNumber is valid for CRUD and return a list of errors,
	 * if any
	 *
	...
	protected List<OHExceptionMessage> checkReferenceNumber(String referenceNumber) throws OHServiceException {
		...
		if (referenceNumber == null || referenceNumber.isEmpty()) {
			...
			if (refNoExists(referenceNumber)) {
				errors.add(new OHExceptionMessage(MessageBundle.getMessage(
						"angal.medicalstock.multiplecharging.theinsertedreferencenumberalreadyexists.msg")));
			}
		}
		return errors;
	}
	 ...
	 * expiring first on top, zero quantities will be stripped out.
	 * 
	...
	/**
	 * Retrieves all the {@link Lot} associated to the specified {@link Medical},
	 * expiring first on top, zero quantities will be stripped out if
	 ...
	 *                        if {@link null}, each movements must have a different
	 *                        referenceNumber
	...
	public List<Movement> newMultipleChargingMovements(List<Movement> movements, String referenceNumber)
			throws OHServiceException {

		// TODO: verify the need of this checkReference in the whole class
		boolean checkReference = referenceNumber == null; // referenceNumber == null, each movement should have
															// referenceNumber set
		if (!checkReference) {
			// referenceNumber != null, all movement will have same referenceNumber, we
			// check only once for all
	 ...
	 * @param checkReference - if {@code true} every movement must have unique
	 *                       reference number
	 ...
	 */
	@Transactional(rollbackFor = OHServiceException.class)
	public List<Movement> newMultipleDischargingMovements(List<Movement> movements, String referenceNumber)
			throws OHServiceException {
	 ...
	 * @throws OHServiceException
	 */
	private List<Movement> prepareDishargingMovement(Movement movement, boolean checkReference)
			throws OHServiceException {
		validateMovement(movement, checkReference);
		if (isAutomaticLotOut()) {
