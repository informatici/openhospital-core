...
public interface AccountingBillIoOperationRepository extends JpaRepository<Bill, Integer> {
	...
	@Modifying
	@Query(value = "update Bill b set b.status='D' where b.id = :billId and b.active = 1")
	void updateDeleteWhereId(@Param("billId") Integer billId);

	@Query(value = "select b from Bill b where b.date >= :dateFrom and b.date < :dateTo and b.active = 1")
	List<Bill> findByDateBetween(@Param("dateFrom") LocalDateTime dateFrom, @Param("dateTo") LocalDateTime dateTo);

	@Query(value = "select b from Bill b where b.billPatient.id = :patientCode and b.date >= :dateFrom and b.date < :dateTo and b.active = 1")
	...
	@Query(value = "select b from Bill b where b.status='O' and b.billPatient.id = :patID and b.active = 1")
	 ...
	 */
	@Query(value = "select bi.bill from BillItems bi where bi.itemDescription = :desc and bi.bill.date >= :dateFrom and bi.bill.date < :dateTo and bi.bill.active = 1")
	...
	@Query(value = "select distinct b.user FROM Bill b WHERE b.active = 1 ORDER BY b.user asc")
