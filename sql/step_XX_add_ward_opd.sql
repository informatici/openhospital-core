...
ALTER TABLE OH_OPD ADD OPD_SCHEDULED_VISIT_ID INT(11) NULL AFTER OPD_USR_ID_A; -- to be taken from previous opd, if any
ALTER TABLE OH_OPD ADD OPD_NEXT_VISIT_ID INT(11) NULL AFTER OPD_SCHEDULED_VISIT_ID; -- the new field

-- update new fields
UPDATE OH_OPD SET OPD_NEXT_VISIT_ID = (SELECT VST_ID FROM OH_VISITS WHERE VST_WRD_ID_A = 'OPD' AND VST_PAT_ID = OPD_PAT_ID AND VST_DATE = OPD_DATE_NEXT_VISIT);
UPDATE OH_OPD SET OPD_SCHEDULED_VISIT_ID = (SELECT VST_ID FROM OH_VISITS WHERE VST_WRD_ID_A = 'OPD' AND VST_PAT_ID = OPD_PAT_ID AND DATE(VST_DATE) <= DATE(OPD_DATE) ORDER BY VST_ID DESC LIMIT 1);
...
ADD CONSTRAINT FK_OPD_NEXT_VISIT
  ...
  ON DELETE SET NULL
  ON UPDATE CASCADE,
ADD CONSTRAINT FK_OPD_SCHED_VISIT
  FOREIGN KEY (OPD_SCHEDULED_VISIT_ID)
  REFERENCES OH_VISITS (VST_ID)
  ON DELETE SET NULL
  ON UPDATE CASCADE;
